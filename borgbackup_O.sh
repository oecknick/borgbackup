...
echo "Beginn Backup_O - "$(date) >> borglog.txt
...
/usr/bin/borg break-lock $REPOSITORY_O
...
/usr/bin/borg create -vspC $COMPRESSION $REPOSITORY_O::Daten_in_O-{now:%Y-%m-%d-%T} /mnt/Laufwerk_O  2>> /var/log/borgbackup/borbackup.log
...
if [ $? = 0 ]
	...
	/usr/bin/mail -s "BorgBackup LW-O-Sicherungs-Script" admin@g-m-l.de < textdateien/erfolg_O.txt
	else
	#echo Hatta nich so gut gemacht
	/usr/bin/mail -s "BorgBackup LW-O-Sicherungs-Script" admin@g-m-l.de < textdateien/misserfolg_O.txt
fi


#Zeitstempel Ende in Logdatei eintragen
echo "Ende Backup_O - "$(date) >> borglog.txt
